...
(function(){
  ...
  function updateControls(){var p=project(),ready=Boolean(p&&p.document);['exportSvgBtn','segmentBtn','directorReviewBtn','adaptBtn','compareVersionsBtn','restoreVersionBtn'].forEach(function(id){var el=$(id);if(el)el.disabled=!ready;});if(!ready)return;var current=p.state&&p.state.currentVersion?p.state.currentVersion:p.document.version;var max=p.state&&p.state.maxVersion?p.state.maxVersion:current;$('versionTo').value=String(current);$('versionTo').max=String(max);$('versionFrom').max=String(max);if(Number($('versionFrom').value)>=current)$('versionFrom').value=String(Math.max(1,current-1));}
  ...
  function bind(){var svg=$('exportSvgBtn'),segmentButton=$('segmentBtn'),director=$('directorReviewBtn'),adaptButton=$('adaptBtn'),compare=$('compareVersionsBtn'),restore=$('restoreVersionBtn');if(svg)svg.onclick=exportSvg;if(segmentButton)segmentButton.onclick=segment;if(director)director.onclick=directorReview;if(adaptButton)adaptButton.onclick=adapt;if(compare)compare.onclick=compareVersions;if(restore)restore.onclick=restoreVersion;var meta=$('designMeta');if(meta&&window.MutationObserver)new MutationObserver(updateControls).observe(meta,{childList:true,subtree:true});updateControls();}
  bind();
})();
</script>`;
