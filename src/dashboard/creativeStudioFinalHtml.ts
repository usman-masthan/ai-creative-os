...
export function creativeStudioFinalHtml(): string {
  ...
  html = injectAfter(
    ...
    '<button id="visualQaBtn" disabled>Visual QA</button><button id="parityBtn" disabled>Parity</button><span id="approvalBadge" class="pill">Unapproved</span><button id="approveVersionBtn" disabled>Approve Version</button><button id="approvedExportBtn" class="primary" disabled>Approved PNG</button>',
  );
...
(function(){
  ...
  function setApprovalUi(data){var approved=Boolean(data&&data.approval);var visualPass=Boolean(data&&data.finalVisualQa&&data.finalVisualQa.decision==='PASS');$('approvalBadge').textContent=approved?('Approved v'+data.version):visualPass?('QA PASS v'+data.version):('Unapproved v'+(data?data.version:'?'));$('approvalBadge').className='pill '+(approved?'ok':visualPass?'warn':'block');$('approveVersionBtn').disabled=!visualPass||approved;$('approvedExportBtn').disabled=!approved;}
  async function refreshApproval(){var id=designId();if(!id){$('approvalBadge').textContent='Unapproved';$('approveVersionBtn').disabled=true;$('approvedExportBtn').disabled=true;return;}try{var data=await api('/api/studio/approval?designId='+encodeURIComponent(id));setApprovalUi(data);}catch(error){setApprovalUi(null);}}
